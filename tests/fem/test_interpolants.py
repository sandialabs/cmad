"""Unit tests for linear hex, tet, quad, and tri interpolants.
...
class TestQuadLinear(unittest.TestCase):
    _degrees = tuple(range(1, 8))
    ...
    def test_partition_of_unity(self):
        for degree in self._degrees:
            ...
            for idx in range(rule.xi.shape[0]):
                shapes = quad_linear(jnp.asarray(rule.xi[idx]))
                with self.subTest(degree=degree, ip=idx):
                    self.assertAlmostEqual(
                        float(jnp.sum(shapes.N)), 1.0, places=12)
    ...
    def test_gradient_sum_to_zero(self):
        for degree in self._degrees:
            ...
            for idx in range(rule.xi.shape[0]):
                ...
                with self.subTest(degree=degree, ip=idx):
                    self.assertTrue(np.allclose(
                        grad_sum, np.zeros(2), atol=1e-12))


class TestTriLinear(unittest.TestCase):
    ...
    def test_partition_of_unity(self):
        for degree in self._degrees:
            ...
            for idx in range(rule.xi.shape[0]):
                shapes = tri_linear(jnp.asarray(rule.xi[idx]))
                with self.subTest(degree=degree, ip=idx):
                    self.assertAlmostEqual(
                        float(jnp.sum(shapes.N)), 1.0, places=12)
