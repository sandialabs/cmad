...
and the indefinite two-field solve end to end, with the direct solver and
...
from cmad.fem.dof import GlobalDofMap, GlobalFieldLayout, build_dof_map
...
def _build_patch_problem() -> tuple[FEProblem, GlobalDofMap, FEState, dict]:
    """Mixed elastic patch problem: u = A·x prescribed on every face."""
    mesh = StructuredHexMesh(lengths=(1.0, 1.0, 1.0), divisions=(3, 3, 3))
    ...
    return fe_problem, dof_map, state, params_by_block


def _assert_patch_solution(
        testcase: unittest.TestCase, fe_problem: FEProblem,
        dof_map: GlobalDofMap, U_solved: JaxArray,
) -> None:
    """u = A·x to roundoff, and the uniform pressure p = -kappa·tr(eps)."""
    L2, H1 = l2_h1_errors(
        fe_problem, U_solved,
        u_exact=lambda coords: _A @ np.asarray(coords),
        grad_u_exact=lambda _coords: _A,
    )
    testcase.assertLess(L2, 1e-9, f"L2 {L2}")
    testcase.assertLess(H1, 1e-9, f"H1 {H1}")
...
class TestMixedUpPatch(unittest.TestCase):
    def test_constant_strain_patch(self) -> None:
        fe_problem, dof_map, state, params_by_block = _build_patch_problem()
        U_solved, _ = fe_newton_solve(
            fe_problem, params_by_block, U_prev=state.U_at(0), t=1.0,
        )
        _assert_patch_solution(self, fe_problem, dof_map, U_solved)

    def test_constant_strain_patch_block_solver(self) -> None:
        """The block preconditioner reproduces the patch solution.

        The same problem solved with GMRES and the block preconditioner
        instead of the direct solver, confirming it solves the indefinite
        coupled system correctly.
        """
        fe_problem, dof_map, state, params_by_block = _build_patch_problem()
        U_solved, _ = fe_newton_solve(
            fe_problem, params_by_block, U_prev=state.U_at(0), t=1.0,
