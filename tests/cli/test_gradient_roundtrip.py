"""Cross-strategy ``cmad gradient`` round-trip.

Builds Calibration data from the truth model, then evaluates
``cmad gradient`` against a deck whose active parameters sit away from
truth (so J and grad are non-trivial) for each of the four
``sensitivity.type`` values, and asserts J and grad agree across
strategies. Mirrors the agreement test in
...
class TestGradientRoundTrip(unittest.TestCase):
    def test_cross_strategy_agreement(self) -> None:
        num_steps = 30
        max_alpha = 0.5
        ...
        strategies = ["adjoint", "direct", "direct_adjoint", "jvp"]
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for stype in strategies:
                ...
                # direct_adjoint with gradient prints a stderr warning;
                # swallow it so the test log stays clean.
                with redirect_stderr(io.StringIO()):
                    exit_code = cmad_main(
                        ["gradient", str(deck_path)],
                    )
                ...
                results[stype] = (J_val, grad)

        ref_J, ref_grad = results[strategies[0]]
        for s in strategies[1:]:
            J_s, grad_s = results[s]
            np.testing.assert_allclose(J_s, ref_J, rtol=1e-9, atol=1e-12)
            np.testing.assert_allclose(
                grad_s, ref_grad, rtol=1e-8, atol=1e-9,
            )
