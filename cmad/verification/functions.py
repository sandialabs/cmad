...
# normal of this is not jax safe due to symmetry assumption
...
# written in terms of full cauchy to be jax safe
...
def jax_unpack_barlat_params(barlat_params):
    ...
    L_sp = jnp.vstack((jnp.c_[L_sp_upper_left, jnp.zeros((3, 3)), jnp.zeros((3, 3))],
                       jnp.c_[jnp.zeros((3, 3)), L_sp_lower_right, jnp.zeros((3, 3))],
                       jnp.c_[jnp.zeros((3, 3)), jnp.zeros((3, 3)), L_sp_lower_right]))
    ...
    L_dp = jnp.vstack((jnp.c_[L_dp_upper_left, jnp.zeros((3, 3)), jnp.zeros((3, 3))],
                       jnp.c_[jnp.zeros((3, 3)), L_dp_lower_right, jnp.zeros((3, 3))],
                       jnp.c_[jnp.zeros((3, 3)), jnp.zeros((3, 3)), L_dp_lower_right]))
