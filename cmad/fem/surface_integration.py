...
def _resolve_sides(
        mesh: Mesh, sides: str | NDArray[np.intp],
) -> NDArray[np.intp]:
    """The ``(elem_id, local_side_id)`` pairs to integrate over."""
    if isinstance(sides, str):
        if sides not in mesh.side_sets:
            raise KeyError(
                f"sideset '{sides}' not in mesh.side_sets (known: "
                f"{sorted(mesh.side_sets)})"
            )
        pairs = mesh.side_sets[sides]
    else:
        pairs = np.asarray(sides, dtype=np.intp)
        if pairs.ndim != 2 or pairs.shape[1] != 2:
            raise ValueError(
                f"sides must be (elem_id, local_side_id) pairs of shape "
                f"(n, 2); got {tuple(pairs.shape)}"
            )
    if pairs.shape[0] == 0:
        raise ValueError("no sides to integrate over")
    return pairs
...
def build_surface_integration_groups(
    ...
    """Precompute per-facet surface geometry for ``field_name`` on a surface.

    ``sides`` names a sideset or gives its ``(elem_id, local_side_id)``
    pairs directly, which lets a caller integrate over part of a face
    rather than a set the mesh happens to carry a name for.

    Partitions the pairs by ``(element family, local side id)``; per
    partition, lifts the side quadrature points to reference volume
    coordinates, evaluates the geometric and field interpolants, and
    forms the surface area element ``dA``, the side shape values
    ``N_side``, and the gather indices ``eq``.
    ...
    pairs = _resolve_sides(mesh, sides)
