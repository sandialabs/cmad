...
def coordinate_side_sets(
    ...
    """Build side sets for the bounding box sides, by coordinate extreme.

    A side (a face for a 3D element, an edge for a 2D one) lies on a
    bounding box extreme when all of its vertices share that extreme
    coordinate, within a per-axis tolerance of ``rel_tol`` times the axis
    extent. Returns the ``{x,y,z}{min,max}_sides`` (``x`` / ``y`` only for
    a 2D mesh) that have at least one such side, as
    ``(elem_id, local_side_id)`` pairs matching :attr:`Mesh.side_sets`; an
    extreme with no sides is dropped rather than returned empty, so the
    result lists only the sides the mesh actually has (a slab gives the two
    ``z`` sides, a rounded body none).

    ``mesh`` is not modified; the caller merges the result into its
    ``side_sets``.
    """
    local_sides = _LOCAL_SIDES_PER_ELEMENT[mesh.element_family]
    side_nodes = mesh.connectivity[:, local_sides]   # (n_elems, n_sides, n_verts)
    side_coords = mesh.nodes[side_nodes]             # (..., n_verts, ndims)
    ...
    for axis, (name_lo, name_hi) in enumerate(name_pairs):
        coord = side_coords[..., axis]               # (n_elems, n_sides, n_verts)
        for name, plane in ((name_lo, lo[axis]), (name_hi, hi[axis])):
            on_plane = np.all(np.abs(coord - plane) < tol[axis], axis=2)
            sides = np.column_stack(np.nonzero(on_plane)).astype(np.intp)
            if sides.shape[0] > 0:
                side_sets[name] = sides
    return side_sets
