...
def l1_penalty(x, alpha, beta):
    return alpha * jnp.sum(jnp.sqrt(x**2 + beta))
...
class EffectiveStressObjective():
    def __init__(self, parameters, sigma_c_values, ratio_c_values, rotations,
        ...
        self._has_regularization = False
        if regularization_fun is not None:
            self._has_regularization = True
            self._regularization = jit(regularization_fun)
            self._regularization_deriv = jit(grad(regularization_fun))
    ...
    def _evaluate(self):
        ...
        if self._has_regularization:
            regularization = \
                self._regularization(self.parameters.flat_active_values())
            J += float(regularization)

            regularization_grad = \
                self._regularization_deriv(self.parameters.flat_active_values())
            dJ_dp += np.asarray(regularization_grad)

        dJ_dp = dJ_dp.squeeze()
        self.parameters.transform_grad(dJ_dp)

        return J, dJ_dp


def calibrate_hill(params, train_data, val_data, weights,
        yield_and_normal_fun, regularization_fun=None):
    ...
    obj = EffectiveStressObjective(params, train_sigma_c_values,
        ...
        yield_and_normal_fun, regularization_fun)
...
def run_hill(params, val_data, yield_and_normal_fun):
...
total_val_sigma_c_values = np.r_[-val_sigma_c_values, val_sigma_c_values]
total_val_data = (total_val_sigma_c_values,
    np.r_[val_R_matrices, val_R_matrices])
total_train_sigma_c_values = np.r_[-train_sigma_c_values, train_sigma_c_values]
total_train_data = (total_train_sigma_c_values, np.r_[train_ratio_c_values,
    train_ratio_c_values],
    np.r_[train_R_matrices, train_R_matrices])

input_scaler = make_input_scaler(total_train_sigma_c_values,
    np.r_[train_R_matrices, train_R_matrices])
#input_scaler.scale_ *= 0.8
input_scaler.min_ *= 0. # important for input symmetric ICNN
#input_scaler.scale_[:] = 1.
output_scaler = make_output_scaler(total_train_sigma_c_values)
#output_scaler.min_ = 0. # causes problems
#output_scaler.scale_ = 1.
...
phi_disc_nn = InputConvexNeuralNetwork(layer_widths,
    input_scaler=input_scaler, output_scaler=output_scaler, seed=22)
...
l1_alpha = 0.
l1_beta = 1e-4
#pt_regularization_fun = partial(l1_penalty, alpha=l1_alpha, beta=l1_beta)
pt_regularization_fun = None

held_out_params, held_out_phi, held_out_ratio = \
    calibrate_hill(hybrid_params, train_data, total_val_data, weights,
    hybrid_yield_and_normal_fun,
    pt_regularization_fun)

nn_props = {}
nn_props["params"] = hybrid_params.values
...
if make_plot:
    ...
    ax[0].scatter(spec_num, paper_phi, color="white", edgecolor="red",
        zorder=1, label="Hill")
    ax[0].scatter(spec_num, held_out_phi[:12], color="white", edgecolor="blue",
    ...
    ax[1].scatter(spec_num, paper_ratio, color="white", edgecolor="red",
        zorder=0, label="Hill")
    ax[1].scatter(spec_num, held_out_ratio[12:], color="white",
        edgecolor="blue", zorder=2, label="Neural Network")
