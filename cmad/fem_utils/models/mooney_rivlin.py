...
class Mooney_rivlin(Global_residual):
    ...
    def _compute_residual_u_p(
        ...
        for gaussPt3D in range(len(gauss_weights_3D)):
            ...
            stress = self._compute_mooney_rivlin_stress_p(grad_u_q, p_q, params)
    ...
    def _compute_residual_u(
        ...
        for gaussPt3D in range(len(gauss_weights_3D)):
            ...
            stress = self._compute_mooney_rivlin_stress(grad_u_q, params)
    ...
    @staticmethod
    def _compute_mooney_rivlin_stress_p(grad_u, p, params):
        ...
        b = F @ F.T
        I1 = jnp.trace(b)
        I2 = 1 / 2 * (jnp.trace(b) ** 2 - jnp.trace(b @ b))

        T = 2 * (C10 + I1 * C01) * b - 2 * C01 * b @ b \
            - (2 / 3 * (C10 * I1 + 2 * C01 * I2) - p) * jnp.eye(3)
        P = T @ F_inv_T

        return P

    @staticmethod
    def _compute_mooney_rivlin_stress(grad_u, params):
        ...
        b = F @ F.T
        b_bar = J ** (-2 / 3) * b
        I1_bar = J ** (-2 / 3) * jnp.trace(b)
        I2_bar = J ** (-4 / 3) * 1 / 2 * (jnp.trace(b) ** 2 - jnp.trace(b @ b))
        
        T = 2 / J * (C10 + C01 * I1_bar) * b_bar - 2 * C01 / J * b_bar @ b_bar \
            + (2 / D1 * (J - 1) - 2 * I1_bar * C10 / (3 * J) \
            - 4 * I2_bar * C01 / (3 * J)) * jnp.eye(3)
